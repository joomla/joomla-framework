...
abstract class Rules
{
	...
	public static function assetFormWidget($actions, $assetId = null, $parent = null, $control = 'jform[rules]', $idPrefix = 'jform_rules')
	{
		...
		foreach ($actions as $action)
		{
			...
			$html[] = '			<caption>' . Text::_('JLIB_HTML_ACCESS_MODIFY_DESC_CAPTION_ACL') . ' ' . $actionTitle . ' '
			...
			foreach ($groups as $i => $group)
			{
				...
				$html[] = '					<select id="' . $idPrefix . '_' . $action->name . '_' . $group->value
					...
					. Text::sprintf('JLIB_RULES_SELECT_ALLOW_DENY_GROUP', $actionTitle, $group->text) . '">';
				$html[] = '						<option value=""' . ($selected === null ? ' selected="selected"' : '') . '>'
					. Text::_('JLIB_RULES_INHERIT') . '</option>';
	...
	{
		$images['allow-l'] = '<label class="icon-16-allow" title="' . Text::_('JLIB_RULES_ALLOWED') . '">' . Text::_('JLIB_RULES_ALLOWED')
			. '</label>';
